...
import util.tools.AndroidTools.OnPost;
...
public class ChatAc extends BaseAc implements CallInt, View.OnClickListener{
	...
	public void callback(String jsonstr) {
		...
		switch (cmd) {
		case MSG.GET_USER_GROUP_CHAT_BY_TYPE_ID_START://查询消息记录 服务器决定一次多x<10>条消息
			...
			break;
		case MSG.SEND_CHATMSG_BY_GTYPE_TOID_TYPE_TIME_MSG://在线时收到一些条消息，判定，是此会话的fromid和toid才显示在当前界面
			...
			//我和目标用户之间的对话消息，此处一条消息，只能是别人发给我，或者别人发给群
				if(this.type.equals("user")){
					if( (Tools.getList(list, 0, "FROMID").equals(this.toid) && Tools.getList(list, 0, "TOID").equals(Constant.id)) ){
						//目标用户发给我，
					...
					if( ( Tools.getList(list, 0, "TOID").equals(this.toid)) ){
						listChatMsg.addAll(listChatMsg.size(), list);
						downLoadFile(list);	//下载需要用的文件####################
			...
			public void call(final int position) {
				...
				if(type.equals("text")){	//文本消息点击,头像点击
					...
					if(Tools.getMapString(map, "isplay").equals("true")){//停止播放
						...
						media.play(Constant.dirVoice + Tools.getMap(map, "MSG"),new MyMediaPlayer.OnPlay() {
							@Override
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {  
         if (requestCode == ACTIVITY_RESULT_CAMERA && resultCode == Activity.RESULT_OK  ) {   
        	Tools.log("拍照结果"+ChatAc.TAKEPHOTO);
	...
	private void setByMap(Map<String, Object> map) {
		if(map != null){
			...
			tvTitleTwo.setText(Tools.getMap(map, "STATUS").toString().equals("null")?"":Tools.getMap(map, "STATUS").toString());
	...
	public void call(int id) { 
		...
		case R.id.ivmenu://用户详情
			...
			AndroidTools.putMapToIntent(intent, MainContactAc.listItems.get( Tools.getCountListByName(MainContactAc.listItems, "ID", this.toid)));
  ...
  //自己发送消息时更新本地会话列表
	public void updateSession(String toid){
		int c = Tools.getCountListByName(MainMsgAc.listSessions, "ID", toid);
		if(c >= 0){
			if(listChatMsg.size() > 0){
				MainMsgAc.listSessions.get(c).put("MSG", listChatMsg.get(listChatMsg.size() - 1).get("MSG").toString());
				MainMsgAc.listSessions.get(c).put("MSGTYPE", listChatMsg.get(listChatMsg.size() - 1).get("TYPE").toString());
	...
	public void sendVoice(final String file){
		...
		map.put("count", Tools.calcTimeSize(media.getDutation(file)));	//该文件的时长
		...
		for(int i = 0; i < list.size(); i++){
			...
			try{
				if(Tools.getMap(map, "TYPE").toString().equals("text")){
					...
					if(Tools.fileExist(Constant.dirVoice + filename)){

						Tools.out("声音文件:" + Constant.dirVoice + filename + "已存在");
						map.put("isok", "true");	//该文件的可用状态
						map.put("count", Tools.calcTimeSize(media.getDutation(Constant.dirVoice + filename)));	//该文件的时长
					}else{
					...
					if(Tools.fileExist(Constant.dirPhoto + filename)){
					...
					if(Tools.fileExist(Constant.dirFile + filename)){ ///下载的文件并重命名，去掉102OTOTO服务器上传下载编制前缀
